...
- [x] CI pipeline — `.github/workflows/ci.yml`: backend (`pytest`) and frontend (`lint`/`build`/`test`) jobs on every PR to `main` and push to `main`. No live services needed (SQLite fixture, mocked LLM calls, lazy Qdrant init from the tests PR). Found and fixed two more pre-existing lint errors while making sure CI would ship green: an `any` cast in `admin/page.tsx` (replaced with `undefined`, the standard axios way to unset a default header) and a `react-hooks/set-state-in-effect` violation in `auth-context.tsx` (the token-decode was moved from a mount effect into a lazy `useState` initializer, so `loading` is no longer needed as real state — the effect that only flipped it to `false` was removed entirely). Also hit two CI-only failures the real Actions run caught (both fixed): `poetry run pytest` invokes the pytest console script directly, not via `python -m`, so it never got the cwd-on-sys.path behavior local `python -m pytest` runs relied on (fixed with a `pythonpath = ["."]` pytest config); and `ChatOpenAI` validates an API key at construction time, so importing `app.graph.nodes` failed outright on the key-less CI runner even though it always worked locally against the real `.env` (fixed with a dummy `OPENAI_API_KEY` in the backend job, never actually called since tests mock all LLM invocations).
